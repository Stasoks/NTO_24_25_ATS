...
coordinates = [
    ...
    [1.846, 3.62, 3.11, 84.37],
    [1.3355, 4.586, 1.4997, 0]
]

# coordinates = [
#     [1.3355, 4.586, 1.5997, 97.03],
# ]
...
def callback(event): 
    ...
    if event == CallbackEvent.ENGINES_STARTED:
        ...
        if position_number < len(coordinates): 
            ap.goToLocalPoint(coordinates[position_number][0],
                            coordinates[position_number][1],
                            coordinates[position_number][2], 5)
            
            if coordinates[position_number][3] != 0:
                ap.updateYaw(coordinates[position_number][3], 1.5)
            sleep(0.5)
            detected = []
            for _ in range(30):
                detected = detect_model(model,  get_img())[0]
                if detected:
                    break
                sleep(0.1)
